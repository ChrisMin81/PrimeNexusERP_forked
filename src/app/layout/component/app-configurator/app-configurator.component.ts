...
type PresetName = KeyOfType<typeof presets>;
...
export class AppConfigurator implements OnInit {
    ...
    updateColors(event: Event, type: ColorType, color: SurfacesType) {
        if (type === 'primary') {
    ...
    applyTheme(type: ColorType, color: SurfacesType) {
        if (type === 'primary') {
    ...
    onPresetChange(event: PresetName | undefined) {
        if (!event) {
            return;
        }
    ...
    onMenuModeChange(event: 'static' | 'overlay' | undefined) {
